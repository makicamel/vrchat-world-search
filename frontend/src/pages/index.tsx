...
}> = ({ worlds, error, loadMoreWorlds, isReachingEnd }): JSX.Element => {
  const loader = <div key="loader">Loading...</div>
  if (!worlds) return loader
  if (error) return <div>An error has occurred.</div>

  const items = (worlds || []).map((world: World, index: number) => {
    const author = (
      <AuthorLink
...
const Home: NextPage = () => {
  const { worlds, error, loadMoreWorlds, isReachingEnd, queries, setQueries } =
    useWorlds()
